...
const PLAN_INFO = {
  ...
  pro: {
    ...
    features: [
      ...
      '10 fontes de dados',
      '5 usuários',
      '100k linhas por dataset',
      '50 perguntas IA por mês',
      'Alertas automáticos',
    ],
  },
  business: {
    ...
    features: [
      ...
      'Datasets ilimitados',
      'Usuários ilimitados',
      '500k linhas por dataset',
      'IA ilimitada',
      'White-label',
    ],
  },
  // legados
  solo:       { name: 'Essential', monthlyPrice: 97.00, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200', features: ['3 dashboards', '3 fontes de dados', '2 usuários'] },
  equipe:     { name: 'Pro',       monthlyPrice: 247.00, color: 'text-violet-600', bg: 'bg-violet-50', border: 'border-violet-200', features: ['15 dashboards', '10 fontes de dados', '5 usuários'] },
  ilimitado:  { name: 'Business',  monthlyPrice: 697.00, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200', features: ['Tudo ilimitado'] },
}
...
function CheckoutContent() {
  ...
  const planKey = searchParams.get('plan') || 'essential'
  ...
  if (!plan) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-500">Plano não encontrado. <Link href="/configuracoes/planos" className="text-violet-600 underline">Voltar aos planos</Link></p>
      </div>
    )
  }
      ...
      <header className="bg-[#0B0A1A] px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <Link
            ...
            className="text-gray-400 hover:text-gray-200 transition-colors text-sm flex items-center gap-1.5"
          >
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-black text-gray-900">
            {step === 'details' ? 'Finalizar assinatura' : 'Dados de pagamento'}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {step === 'details'
              ? 'Revise os detalhes e aplique um cupom antes de prosseguir'
              : 'Insira os dados do cartão para ativar seu plano'}
          </p>
        </div>

        {/* Steps indicator */}
        <div className="flex items-center justify-center gap-3 mb-8">
          <div className={`flex items-center gap-2 text-sm font-semibold ${step === 'details' ? 'text-violet-600' : 'text-gray-400'}`}>
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black ${step === 'details' ? 'bg-violet-600 text-white' : 'bg-gray-300 text-white'}`}>1</span>
            Detalhes
          </div>
          <div className="w-16 h-px bg-gray-300" />
          <div className={`flex items-center gap-2 text-sm font-semibold ${step === 'payment' ? 'text-violet-600' : 'text-gray-400'}`}>
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black ${step === 'payment' ? 'bg-violet-600 text-white' : 'bg-gray-300 text-white'}`}>2</span>
            Pagamento
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider mb-4">Resumo do pedido</p>

            <div className={`rounded-xl border ${plan.border} ${plan.bg} px-4 py-4 mb-5`}>
              <div className="flex items-baseline justify-between">
                <div>
                  <span className="text-xs text-gray-500 font-medium">Plano selecionado</span>
                  <div className={`text-xl font-black mt-0.5 ${plan.color}`}>Jarbis {plan.name}</div>
                </div>
                <div className="text-right">
                  <div className={`text-2xl font-black ${plan.color}`}>
            ...
            <ul className="space-y-2 mb-5">
              {plan.features.map(f => (
                <li key={f} className="flex items-center gap-2.5 text-sm text-gray-600">
                  <svg className={`w-4 h-4 flex-shrink-0 ${plan.color}`} viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {step === 'details' && (
              <form onSubmit={handleDetailsSubmit} className="space-y-5">
                <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider mb-1">Detalhes da assinatura</p>
                ...
                <p className="text-center text-xs text-gray-400">
                  No próximo passo você insere os dados do cartão. Cancele a qualquer momento.
                </p>
              </form>
            )}
            ...
            {step === 'payment' && stripeInstance && clientSecret && (
              <>
                <div className="flex items-center gap-2 mb-5">
                  <button
                    onClick={() => { setStep('details'); setClientSecret(null); setStripeInstance(null) }}
