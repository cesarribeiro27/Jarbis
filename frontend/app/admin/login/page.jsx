...
export default function AdminLoginPage() {
  ...
  async function handleCredentials(e) {
    ...
    try {
      ...
      // 3. Verificar se tem 2FA ativo
      const totpRes = await fetch(`${API_URL}/admin/totp/status`, {
        headers: { 'Authorization': `Bearer ${token}` },
        credentials: 'include',
      })
      const totpData = totpRes.ok ? await totpRes.json() : { totp_enabled: false }

      if (totpData.totp_enabled) {
        // Redireciona para step de OTP — guarda token temporariamente em memória
        setPendingToken(token)
        setStep('otp')
        return
      }

      // Sem 2FA: setar cookie direto
  ...
  async function handleOtp(e) {
    ...
    try {
      const res = await fetch(`${API_URL}/admin/totp/verify`, {
        method: 'POST',
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-950 px-4">
      <div className="w-full max-w-sm">
        ...
        {step === 'credentials' ? (
          ...
          <>
            <h1 className="text-2xl font-black text-white tracking-tight mb-1 text-center">Verificacao em 2 etapas</h1>
            <p className="text-gray-500 text-sm text-center mb-8">Digite o codigo do seu app autenticador</p>
            ...
            <form onSubmit={handleOtp} className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-400 mb-2">Codigo OTP (6 digitos)</label>
                <input
                  type="text"
                  inputMode="numeric"
                  pattern="[0-9]{6}"
                  ...
                  required
                  autoFocus
                  className="w-full bg-gray-900 border border-gray-800 text-white rounded-xl px-4 py-3 text-2xl text-center tracking-widest font-mono focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                  placeholder="000000"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-violet-600 text-white font-bold py-3.5 rounded-full hover:bg-violet-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                type="button"
                onClick={() => { setStep('credentials'); setError(''); setPendingToken(null) }}
