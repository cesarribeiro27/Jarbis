...
function ApiDatasetModal({ onClose, onCreated }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      onCreated(ds)
      onClose()
      toast('Dataset conectado com sucesso!', 'success')
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Importe dados de qualquer endpoint REST</p>
          </div>
        ...
        <form onSubmit={submit} className="p-6 flex flex-col gap-4">
          <div>
            ...
            <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Ex: Vendas por mês" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" autoFocus />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-500 mb-1.5">URL da API</label>
              <input required type="url" value={form.api_url} onChange={e => setForm(f => ({ ...f, api_url: e.target.value }))} placeholder="https://api.exemplo.com/dados" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1.5">Atualizar automaticamente a cada (minutos)</label>
            <input type="number" min="1" value={form.refresh_interval_minutes} onChange={e => setForm(f => ({ ...f, refresh_interval_minutes: e.target.value }))} placeholder="Ex: 60 (deixe vazio para não atualizar)" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full px-4 py-2.5 bg-violet-600 text-white text-sm font-semibold rounded-lg hover:bg-violet-700 disabled:opacity-50 transition-colors">
            {loading ? 'Conectando...' : 'Conectar e importar dados'}
          </button>
        </form>
      </div>
    </div>
...
export default function DatasetsPage() {
  ...
  useEffect(() => {
    api.reports.datasets.list()
      ...
      .catch(() => toast('Erro ao carregar datasets.', 'error'))
      .finally(() => setLoading(false))
  }, [])
  ...
  async function handleUpload(file) {
    ...
    if (!['csv', 'xlsx', 'xls'].includes(ext)) {
      toast('Formato inválido. Use CSV ou Excel (.xlsx/.xls)', 'error')
      return
    }
    if (file.size > 50 * 1024 * 1024) {
      toast('Arquivo muito grande. Limite: 50MB', 'error')
      return
    }
    setUploading(true)
    ...
    try {
      ...
      toast(`"${ds.name}" importado com sucesso! ${ds.row_count?.toLocaleString('pt-BR') || 0} linhas.`, 'success')
    } catch (err) {
  ...
  async function handleSync(id) {
    ...
    try {
      ...
      toast('Dataset sincronizado com sucesso!', 'success')
    } catch (err) {
  ...
  async function handleDelete(id) {
    try {
      ...
      toast('Dataset excluído.', 'success')
    } catch (err) {
      ...
      <div className="p-6 max-w-screen-xl mx-auto">
        ...
        {!loading && datasets.length === 0 && (
          ...
          >
            ...
            <p className="font-semibold text-gray-800 mb-2">{dragOver ? 'Solte para importar' : 'Nenhum dataset ainda'}</p>
            <p className="text-sm text-gray-400 mb-1">Arraste um arquivo aqui ou clique para fazer upload</p>
            <p className="text-xs text-gray-300 mb-6">Suporta CSV e Excel · Limite 50MB</p>
        ...
        {!loading && datasets.length > 0 && (
          ...
          >
            ...
            >
              {dragOver ? 'Solte para importar' : '↑ Arraste um arquivo CSV/Excel aqui para importar'}
            </div>
          </div>
        )}
      </div>
