...
function GADatasetModal({ onClose }) {
  const [selectedTemplate, setSelectedTemplate] = useState('acquisition')
  const [form, setForm] = useState({ name: '', property_id: '', date_range_days: 90 })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  const tpl = GA_TEMPLATES.find(t => t.id === selectedTemplate)

  const handleConnect = async () => {
    if (!form.property_id) { setError('Informe o Property ID.'); return }
    setLoading(true)
    setError('')
    try {
      const name = form.name.trim() || tpl.label
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#1E1E2E] border border-[#E2E8F0] dark:border-[#2D2D3D] rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-[#E2E8F0] dark:border-[#2D2D3D]">
          <h2 className="text-base font-bold text-[#1A1A2E] dark:text-[#F1F5F9] flex items-center gap-2 mb-0.5">
          ...
          <p className="text-xs text-[#6B7280] dark:text-[#94A3B8]">Selecione o tipo de análise e autorize o acesso com sua conta Google.</p>
        </div>

        <div className="px-6 py-4 space-y-4 max-h-[60vh] overflow-y-auto">
          {/* Templates */}
          <div>
            <p className="text-xs font-semibold text-[#1A1A2E] dark:text-[#F1F5F9] mb-2">Tipo de análise</p>
            <div className="space-y-2">
              {GA_TEMPLATES.map((t, i) => (
                <button
                  key={t.id}
                  onClick={() => { setSelectedTemplate(t.id); setForm(f => ({...f, name: t.label})) }}
                  className={`w-full text-left p-3 rounded-xl border transition-all ${selectedTemplate === t.id ? 'border-[#6D28D9] bg-[#f5f3ff] dark:bg-[#6D28D9]/10' : 'border-[#E2E8F0] dark:border-[#2D2D3D] hover:border-[#7C3AED] hover:bg-[#f5f3ff] dark:hover:bg-[#6D28D9]/5'}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${selectedTemplate === t.id ? 'bg-[#6D28D9]' : 'bg-[#E2E8F0] dark:bg-[#2D2D3D]'}`} />
                    <span className={`text-xs font-semibold ${selectedTemplate === t.id ? 'text-[#6D28D9]' : 'text-[#1A1A2E] dark:text-[#F1F5F9]'}`}>
                      {i === 0 && <span className="mr-1.5 text-[10px] bg-[#6D28D9] text-white px-1.5 py-0.5 rounded-full font-bold">Mais importante</span>}
                      {t.label}
                    </span>
                  </div>
                  <p className="text-xs text-[#6B7280] dark:text-[#94A3B8] pl-3.5">{t.description}</p>
                  {selectedTemplate === t.id && (
                    <div className="flex flex-wrap gap-1 mt-2 pl-3.5">
                      {t.columns.map(c => (
                        <span key={c} className="text-[10px] px-1.5 py-0.5 rounded-md bg-[#6D28D9]/10 text-[#6D28D9] dark:bg-[#6D28D9]/20 dark:text-[#A78BFA] font-medium">{c}</span>
                      ))}
                    </div>
                  )}
                </button>
          ...
          {/* Property ID */}
          <div>
            <label className="block text-xs font-semibold text-[#1A1A2E] dark:text-[#F1F5F9] mb-1">Property ID (GA4)</label>
            <input
              className="w-full border border-[#E2E8F0] dark:border-[#2D2D3D] dark:bg-[#0F0F1A] dark:text-[#F1F5F9] rounded-xl px-3 py-2 text-sm font-mono focus:outline-none focus:border-[#6D28D9] transition-colors"
              placeholder="ex: 123456789"
              value={form.property_id}
              onChange={e => setForm(f => ({...f, property_id: e.target.value}))}
            />
            <p className="text-xs text-[#6B7280] dark:text-[#94A3B8] mt-0.5">Google Analytics → Administrador → Propriedade → ID da propriedade</p>
          </div>

          {/* Período */}
          <div>
            <label className="block text-xs font-semibold text-[#1A1A2E] dark:text-[#F1F5F9] mb-1">Período</label>
            <select
              className="w-full border border-[#E2E8F0] dark:border-[#2D2D3D] dark:bg-[#0F0F1A] dark:text-[#F1F5F9] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#6D28D9] transition-colors"
              value={form.date_range_days}
              onChange={e => setForm(f => ({...f, date_range_days: e.target.value}))}
            >
        ...
        {/* Footer */}
        <div className="flex gap-3 px-6 py-4 border-t border-[#E2E8F0] dark:border-[#2D2D3D]">
          <button onClick={onClose} className="flex-1 border border-[#E2E8F0] dark:border-[#2D2D3D] rounded-xl py-2 text-sm text-[#6B7280] dark:text-[#94A3B8] hover:bg-[#f5f3ff] dark:hover:bg-[#2D2D3D] transition-colors">
            Cancelar
          </button>
          <button
            onClick={handleConnect}
            disabled={loading || !form.property_id}
            className="flex-1 flex items-center justify-center gap-2 bg-[#6D28D9] hover:bg-[#7C3AED] disabled:opacity-50 text-white rounded-xl py-2 text-sm font-semibold transition-colors"
          >
            {loading
