          ...
          {datasets.length === 0 ? (
            ...
            <>
              <div>
                ...
                <select value={datasetId} onChange={e => setDatasetId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && ask()} placeholder="Ex: qual produto vendeu mais?" className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" autoFocus />
                  <button onClick={ask} disabled={loading || !datasetId || !question.trim()} className="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-40 transition-colors">
                    {loading ? '...' : 'Enviar'}
                  </button>
                </div>
              </div>
              {error && <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-sm text-red-600">{error}</div>}
              {result?.answer && <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3"><p className="text-sm text-indigo-900">{result.answer}</p></div>}
            </>
          )}
        </div>
      </div>
    ...
    return (
      <div className="h-screen flex flex-col overflow-hidden bg-gray-50">
        <div className="bg-white border-b border-gray-100 px-4 py-2.5 flex items-center gap-3 shrink-0">
          ...
          <button onClick={() => setShowDateFilter(v => !v)} className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border transition-colors ${showDateFilter || globalDateFilter.dateFrom || globalDateFilter.dateTo ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
        ...
        {showDateFilter && (
          <div className="bg-indigo-50 border-b border-indigo-100 px-4 py-2 flex items-center gap-3 shrink-0 flex-wrap">
            <span className="text-xs font-semibold text-indigo-700 uppercase tracking-wider">Filtro de Data</span>
            <select value={globalDateFilter.dateCol} onChange={e => setGlobalDateFilter(f => ({ ...f, dateCol: e.target.value }))} className="border border-indigo-200 rounded-lg px-2 py-1 text-xs bg-white text-gray-700 focus:outline-none">
            ...
            <input type="date" value={globalDateFilter.dateFrom} onChange={e => setGlobalDateFilter(f => ({ ...f, dateFrom: e.target.value }))} className="border border-indigo-200 rounded-lg px-2 py-1 text-xs bg-white" />
            <span className="text-xs text-indigo-400">até</span>
            <input type="date" value={globalDateFilter.dateTo} onChange={e => setGlobalDateFilter(f => ({ ...f, dateTo: e.target.value }))} className="border border-indigo-200 rounded-lg px-2 py-1 text-xs bg-white" />
            <button onClick={() => setGlobalDateFilter({ dateCol: '', dateFrom: '', dateTo: '' })} className="text-xs text-indigo-500 hover:text-indigo-700">Limpar</button>
          </div>
        )}
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="flex-1 overflow-auto p-6 min-w-0" style={{ backgroundColor: canvasConfig.bgColor || '#f3f4f6' }} onClick={() => setSelectedBlockId(null)}>
            <div className="flex items-center gap-1 mb-4 flex-wrap" onClick={e => e.stopPropagation()}>
              {pages.map(page => (
                <div key={page.id} className={`group flex items-center gap-1 rounded-lg border transition-colors ${activePageId === page.id ? 'bg-white border-indigo-300 shadow-sm' : 'bg-transparent border-transparent hover:border-gray-200'}`}>
                  {renamingPageId === page.id ? (
                    ...
                    <button className={`text-xs font-medium px-2.5 py-1.5 rounded-lg ${activePageId === page.id ? 'text-indigo-700' : 'text-gray-500 hover:text-gray-800'}`} onClick={() => { setActivePageId(page.id); setSelectedBlockId(null) }} onDoubleClick={() => setRenamingPageId(page.id)}>
                      {page.title}
                    </button>
                  )}
              ...
              <button onClick={addPage} className="flex items-center justify-center w-7 h-7 rounded-lg border border-dashed border-gray-300 text-gray-400 hover:border-indigo-400 hover:text-indigo-500 transition-colors text-sm">+</button>
            </div>

            <div className="mb-6" onClick={e => e.stopPropagation()}>
              <input className="text-2xl font-black text-gray-800 bg-transparent outline-none border-b-2 border-transparent focus:border-indigo-300 transition-colors py-1 w-full block" value={editTitle} onChange={e => setEditTitle(e.target.value)} />
          ...
          <div className="w-12 bg-white border-l border-gray-100 flex flex-col items-center py-3 gap-1 shrink-0">
            ...
            <button title="Perguntar com IA" onClick={() => setShowAiPanel(true)} className="w-8 h-8 flex items-center justify-center rounded-lg transition-colors text-gray-400 hover:text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="p-6 max-w-screen-xl mx-auto">
        <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            <button onClick={enterEditMode} className="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-colors">Editar</button>
        ...
        {shareData && (
          <div className="bg-white rounded-xl border border-gray-100 p-4 mb-6 flex items-center gap-3">
            ...
            <button onClick={async () => { await navigator.clipboard.writeText(shareData.share_url); setCopied(true); setTimeout(() => setCopied(false), 2000) }} className="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 transition-colors shrink-0">
        ...
        <div className="mb-4 flex items-center gap-2 flex-wrap">
          <button onClick={() => setShowDateFilter(v => !v)} className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-xl border transition-colors ${showDateFilter || globalDateFilter.dateFrom || globalDateFilter.dateTo ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 2v4M8 2v4M3 10h18" /></svg>
            Filtro de data
          </button>
          {showDateFilter && (
            <>
              <select value={globalDateFilter.dateCol} onChange={e => setGlobalDateFilter(f => ({ ...f, dateCol: e.target.value }))} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400">
                <option value="">Coluna de data...</option>
                {[...new Set(datasets.flatMap(ds => ds.columns || []))].sort().map(col => <option key={col} value={col}>{col}</option>)}
              </select>
              <input type="date" value={globalDateFilter.dateFrom} onChange={e => setGlobalDateFilter(f => ({ ...f, dateFrom: e.target.value }))} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
              <span className="text-sm text-gray-400">até</span>
              <input type="date" value={globalDateFilter.dateTo} onChange={e => setGlobalDateFilter(f => ({ ...f, dateTo: e.target.value }))} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
        ...
        {pages.length > 1 && (
          <div className="flex items-center gap-1 mb-4 flex-wrap border-b border-gray-100 pb-3">
            {pages.map(page => (
              <button key={page.id} onClick={() => setActivePageId(page.id)} className={`px-3 py-1.5 text-sm rounded-xl transition-colors ${activePageId === page.id ? 'bg-indigo-600 text-white font-bold' : 'text-gray-600 hover:bg-gray-100'}`}>
                {page.title}
              </button>
            ))}
          </div>
