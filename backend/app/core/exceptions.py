...
class LumetraException(HTTPException):
    """Base para todas as exceções da plataforma."""
    pass


class NotFoundError(LumetraException):
...
class UnauthorizedError(LumetraException):
...
class ForbiddenError(LumetraException):
...
class ConflictError(LumetraException):
...
class ValidationError(LumetraException):
    def __init__(self, message: str):
        super().__init__(
...
class PlanLimitError(LumetraException):
